...
class Plugin_License {
	...
	private function perform_license_action( string $action, bool $force ) {
		...
		if ( empty( $this->license ) && 'check_license' === $action ) {
			return array(
				...
				'license_valid'  => false,
				'message'        => __( 'It appears the license key is blank.', 'gb-extras' ),
				'data'           => array(),
				'action'         => $action,
			);
		}
		...
		if ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			return new \WP_Error( 'sce_license_communications_error', __( 'We could not communicate with the update server. Please try again later.', 'gb-extras' ) );
		}
		 ...
		 */
		do_action( 'dlx_gb_extras_core_license_' . $action, $response );

		// Return any custom data.
		$data = apply_filters( 'dlx_gb_extras_core_license_data_' . $action, $response );
	...
	private function get_response( array $response, string $action ) {
		...
		if ( ! empty( $expires ) ) {
			...
			if ( 'lifetime' === $expires ) {
				$return_defaults['expires_human_time_diff'] = esc_html__( 'Lifetime', 'gb-extras' );
				$return_defaults['expires']                 = esc_html__( 'Lifetime', 'gb-extras' );
			} else {
		...
		if ( true === $response['success'] ) {
			if ( 'deactivate_license' === $action ) {
				return wp_parse_args(
					array(
						'message' => __( 'Your license key has been deactivated.', 'gb-extras' ),
					),
					$return_defaults
				);
			}
			if ( 'activate_license' === $action || 'check_license' === $action ) {
				return wp_parse_args(
					array(
						'message' => __( 'Your license key is active and valid.', 'gb-extras' ),
					),
					$return_defaults
				);
			}
		...
		// There are errors.
		$error_message = __( 'An error occurred, please try again.', 'gb-extras' );
			...
			switch ( $errors ) {
				case 'expired':
					$error_message = sprintf(
						...
						__( 'Your license key expired on %s.', 'gb-extras' ),
				...
				case 'disabled':
				case 'revoked':
					$error_message = __( 'Your license key has been disabled.', 'gb-extras' );
					break;

				case 'missing':
					$error_message = __( 'The license entered is not valid.', 'gb-extras' );
					break;
				case 'invalid':
				case 'site_inactive':
					$error_message = __( 'Your license is not active for this URL.', 'gb-extras' );
					break;

				case 'item_name_mismatch':
					/* Translators: %s is the plugin name */
					$error_message = sprintf( __( 'This appears to be an invalid license key for %s.', 'gb-extras' ), 'Toggl Plan for Gravity Forms' );
					break;

				case 'no_activations_left':
					$error_message = __( 'Your license key has reached its activation limit.', 'gb-extras' );
					break;
			}
		}
